...
def _parity_error(flags, with_water=True, **methods):
    ...
    ref_list, _ = NIfTI_RawProcessor(volatile=True, **flags, **methods).process_nifti_list(
...
#**************************************************************************************************#
#                                                                                                  #
# Every RawProcessor step against its NIfTI_RawProcessor counterpart.                              #
#                                                                                                  #
#**************************************************************************************************#
class TestTensorParity:
    """Every RawProcessor step against its NIfTI_RawProcessor counterpart."""
    ...
    @pytest.mark.parametrize('method', ['own', 'fsl-mrs'])
    ...
    def test_own_alignment_solves_same_objective(self):
        ...
        ref_list, _ = NIfTI_RawProcessor(volatile=True, **pre).process_nifti_list(mets, wats)
        fids = np.stack([np.squeeze(n[:]).T for n in ref_list])     # (B, D, T)

        costs = {}
        for method in ('fsl-mrs', 'own'):
        ...
        assert np.all(costs['own'] <= costs['fsl-mrs'] * 1.1 + 1e-9)
...
class TestTensorWrapper:
    ...
    def test_wrapper_collapses_dims_and_matches(self):
        ...
        ref_list, _ = NIfTI_RawProcessor(volatile=True).process_nifti_list(mets, wats)
        ref = np.stack([np.squeeze(n[:]) for n in ref_list])
